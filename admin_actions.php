...
function updateUploadStatus($input) {
    ...
    try {
        ...
        if ($success && $stmt->rowCount() > 0) {
            ...
            echo json_encode(['success' => false, 'message' => 'Failed to update status or upload not found']);
        }
    } catch (Exception $e) {
...
function bulkUpdateStatus($input) {
    ...
    try {
        ...
        $stmt = $pdo->prepare("UPDATE uploads SET approval_status = ?, approved_by = ?, updated_at = datetime('now') WHERE id IN ($placeholders)");
