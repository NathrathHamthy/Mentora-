...
if ($_POST && isset($_POST['update_status'])) {
    ...
    if (in_array($newStatus, ['pending', 'approved', 'rejected'])) {
        try {
            $stmt = $pdo->prepare("UPDATE uploads SET approval_status = ?, approved_by = ?, updated_at = datetime('now') WHERE id = ?");

            if ($stmt->execute([$newStatus, $_SESSION['user_id'], $uploadId])) {
                if ($stmt->rowCount() > 0) {
                    $successMessage = "Upload status updated to " . ucfirst($newStatus) . " successfully!";
                    $upload['approval_status'] = $newStatus; // Update the display

                    // Refresh the upload data
                    $stmt = $pdo->prepare("SELECT u.*, users.full_name as uploader_name, users.email as uploader_email FROM uploads u JOIN users ON u.user_id = users.id WHERE u.id = ?");
                    $stmt->execute([$uploadId]);
                    $upload = $stmt->fetch();
                } else {
                    $errorMessage = "No changes were made to the upload status.";
                }
            } else {
                $errorMessage = "Failed to update upload status.";
            }
        } catch (Exception $e) {
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="mt-4">
                    ...
                    <div class="btn-group">
                        <?php 
                        // Check if file exists using proper path resolution
                        ...
                        <?php else: ?>
                            ...
                            <small class="text-muted d-block mt-2">Path: <?php echo htmlspecialchars($upload['file_path']); ?></small>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
